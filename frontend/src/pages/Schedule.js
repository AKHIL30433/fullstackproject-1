  ...
  useEffect(() => {
    if (user?.role === 'admin') {
      ...
      API.get('/applications').then(r => {
        setApplications(r.data.filter(app => app.status === 'approved'));
      }).catch(() => {});
    }
  }, [user]);
  ...
  if (user?.role !== 'admin') {
    return (
      <Layout>
        ...
        <div style={{background:'#f0fdf4',border:'1px solid #bbf7d0',borderRadius:'10px',padding:'20px',marginBottom:'20px',display:'inline-block',minWidth:'160px'}}>
        ...
        <div style={{background:'#fff',border:'1px solid #e5e7eb',borderRadius:'12px',padding:'20px'}}>
          ...
          {applications.length === 0 && (
            <p style={{color:'#9ca3af',textAlign:'center',padding:'32px',fontSize:'14px'}}>No approved applications yet.</p>
          )}
          {applications.map(app => (
            <div key={app._id} style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',padding:'16px 0',borderBottom:'1px solid #f3f4f6'}}>
              <div>
                ...
                <div style={{fontSize:'12px',color:'#9ca3af',marginTop:'4px'}}>Applied: {app.appliedAt ? new Date(app.appliedAt).toLocaleDateString() : '—'}</div>
              </div>
            ...
            <div key={shift._id} style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',padding:'16px 0',borderBottom:'1px solid #f3f4f6'}}>
              <div>
                ...
                <div style={{fontSize:'12px',color:'#9ca3af',marginTop:'4px'}}>
                  📅 {shift.date} &nbsp;·&nbsp; 🕐 {shift.startTime} - {shift.endTime} ({hours}h)
